...
ObsidianWeather is a Paper plugin that adds dangerous, discoverable wind events to Minecraft weather. Tornadoes form during vanilla thunderstorms, firenadoes also roam the Nether, and dust devils cross hot, dry biomes under clear skies. Every event follows a bounded semi-random path, affects configured blocks and entities, and dissipates naturally without server-wide alerts or artificial spawn cooldowns.

## Features

- Six independently tunable Fujita ratings: F0 through the exceptionally rare F5.
- Five behaviorally distinct types:
  - **Standard:** baseline moving funnel, debris, rotational lift, and damage. Each spawn randomly uses a classic cone, rope, stovepipe, wedge, or multi-vortex profile.
  - **Firenado:** forms in hot biomes or near fire/lava during thunder and naturally throughout the Nether; burns entities and starts policy-aware fires.
...
- A configurable per-world safety cap, defaulting to eight active events for large multiplayer worlds.
- Loaded-chunk-only movement and budgeted surface sampling, including cave-level Nether routing.
...
Every `spawn.check-interval-ticks`, each configured world below its event cap gets an independent probability roll. A candidate location is chosen in a loaded chunk near an online player. The plugin filters enabled types by current dimension, weather, and environment, rolls an eligible type by weight, then rolls that type's Fujita rating using its own weights.

Standard tornadoes, overworld firenadoes, icenadoes, and waterspouts require a vanilla thunderstorm. Dust devils instead require clear weather and a hot, dry biome. In the Nether, firenadoes need no impossible thunder condition and the base probability is multiplied by `spawn.nether-firenado-chance-multiplier`, which defaults to `4.0`.

A successful spawn does not alter the next probability check. There is intentionally no “wait N minutes after an event” timer. The per-world cap is only a performance boundary and can be tuned independently of probability.
...
| Command | Description |
|---|---|
| `/obsidianweather spawn <world> [x z] [type] [rating]` | Spawn an event. Omitted type/rating values use weighted selection. Players may omit coordinates; console must provide them. |
| `/obsidianweather stop <id\|all>` | Stop one event by full/partial ID, or all events. |
| `/obsidianweather list` | List IDs, type, Fujita rating, standard profile, world, position, and remaining lifetime. |
| `/obsidianweather reload` | Reload config. Existing events retain their spawn-time stats. |
| `/ow ...` | Short alias. |

Valid types: `standard`, `firenado`, `icenado`, `waterspout`, `dust-devil`. Valid ratings: `f0`, `f1`, `f2`, `f3`, `f4`, `f5`. Commands continue to accept the legacy names `weak`, `moderate`, `strong`, `severe`, and `violent` as aliases for F0–F4.
...
| `spawn.chance-per-check` | `0.004` | Base probability per eligible world and check. |
| `spawn.nether-firenado-chance-multiplier` | `4.0` | Multiplier applied to natural Nether firenado rolls. |
| `spawn.max-concurrent-per-world` | `8` | Hard per-world active safety cap; not a cooldown. |
...
| `physics.base-damage` | `1.5` | Direct damage before rating scaling. |
...
|---|---:|---:|---|---|
| `standard` | `true` | `60.0` | Thunderstorm; any eligible loaded location | Five random visual profiles and baseline vortex. |
| `firenado` | `true` | `15.0` | Hot/fiery thunderstorm location, or anywhere in the Nether | Fire and ignition. |
...
Each type owns `types.<type>.tiers.<rating>`. Fields are `radius`, `movement-speed`, `lifespan-seconds`, `pickup-weight-limit`, `damage-multiplier`, `particle-density`, and `spawn-weight`.

Existing configuration sections named `weak`, `moderate`, `strong`, `severe`, and `violent` remain readable as F0, F1, F2, F3, and F4 respectively. New configurations use `f0` through `f5`.
...
### Complete Fujita defaults

Columns: radius (`R`), movement (`S`), lifespan seconds (`L`), pickup limit (`P`), damage (`D`), particle density (`Fx`), rating weight (`W`). Type-specific movement multipliers apply after `S`.

| Type | Rating | R | S | L | P | D | Fx | W |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| Standard | F0 | 6 | 0.12 | 90 | 2 | 0.65 | 0.65 | 45 |
| Standard | F1 | 8 | 0.15 | 120 | 3 | 0.9 | 0.85 | 30 |
| Standard | F2 | 11 | 0.18 | 150 | 4 | 1.2 | 1.0 | 16 |
| Standard | F3 | 14 | 0.21 | 180 | 6 | 1.6 | 1.25 | 7 |
| Standard | F4 | 18 | 0.24 | 210 | 8 | 2.2 | 1.5 | 2 |
| Standard | F5 | 22 | 0.27 | 240 | 10 | 3.0 | 1.8 | 0.5 |
| Firenado | F0 | 5 | 0.11 | 75 | 2 | 0.8 | 0.7 | 50 |
| Firenado | F1 | 7 | 0.14 | 100 | 3 | 1.05 | 0.9 | 29 |
| Firenado | F2 | 9 | 0.17 | 130 | 4 | 1.4 | 1.1 | 14 |
| Firenado | F3 | 12 | 0.20 | 155 | 6 | 1.9 | 1.35 | 5.5 |
| Firenado | F4 | 15 | 0.23 | 180 | 8 | 2.6 | 1.65 | 1.5 |
| Firenado | F5 | 18 | 0.26 | 210 | 10 | 3.4 | 1.9 | 0.4 |
| Icenado | F0 | 7 | 0.10 | 100 | 2 | 0.55 | 0.75 | 46 |
| Icenado | F1 | 9 | 0.13 | 130 | 3 | 0.8 | 0.95 | 31 |
| Icenado | F2 | 12 | 0.16 | 160 | 4 | 1.1 | 1.15 | 15 |
| Icenado | F3 | 15 | 0.18 | 190 | 6 | 1.45 | 1.4 | 6 |
| Icenado | F4 | 19 | 0.20 | 220 | 8 | 2.0 | 1.7 | 2 |
| Icenado | F5 | 23 | 0.22 | 250 | 10 | 2.6 | 2.0 | 0.5 |
| Waterspout | F0 | 6 | 0.14 | 80 | 0 | 0.7 | 0.7 | 52 |
| Waterspout | F1 | 8 | 0.17 | 105 | 0 | 0.95 | 0.9 | 28 |
| Waterspout | F2 | 10 | 0.20 | 130 | 0 | 1.3 | 1.1 | 13 |
| Waterspout | F3 | 13 | 0.23 | 155 | 0 | 1.75 | 1.35 | 5.5 |
| Waterspout | F4 | 16 | 0.27 | 180 | 0 | 2.3 | 1.6 | 1.5 |
| Waterspout | F5 | 19 | 0.30 | 210 | 0 | 3.0 | 1.9 | 0.4 |
| Dust Devil | F0 | 4 | 0.17 | 45 | 1 | 0.25 | 1.1 | 50 |
| Dust Devil | F1 | 5 | 0.20 | 60 | 2 | 0.4 | 1.25 | 30 |
| Dust Devil | F2 | 7 | 0.23 | 75 | 2 | 0.6 | 1.4 | 14 |
| Dust Devil | F3 | 9 | 0.26 | 90 | 3 | 0.85 | 1.6 | 5 |
| Dust Devil | F4 | 11 | 0.30 | 105 | 3 | 1.15 | 1.8 | 1 |
| Dust Devil | F5 | 14 | 0.34 | 120 | 4 | 1.5 | 2.0 | 0.25 |
